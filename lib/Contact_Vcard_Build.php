...
// +----------------------------------------------------------------------+ 
// 
// $Id: Contact_Vcard_Build.php 56 2008-11-04 16:51:03Z  $ 
...
* @author Gilles Doge <gde@antistatique.net> (nov. 2008) PHP5 support and remove dependancy with PEAR.php (only used for handling error, now it's Exception)
* 
...
class Contact_Vcard_Build {
    ...
    */
    ...
    function escape(&$text)
    {
        if (is_object($text)) {
            ...
            throw new Exception('The escape() method works only with string literals and arrays.');
    ...
    */
        ...
        $iter = null)
    {
        ...
        if (! is_integer($iter) || $iter < 0) {
            ...
            throw new Exception("$iter is not a valid iteration number for $comp; must be a positive integer.");
    ...
    */
    ...
    function validateParam($name, $text, $comp = null, $iter = null)
    {
        ...
        if (preg_match('/[^a-zA-Z0-9\-]/i', $text)) {
            ...
            throw new Exception("vCard [$comp] [$iter] [$name]: The parameter value may contain only a-z, A-Z, 0-9, and dashes (-).");
            ...
            case 'TYPE':
                if (! in_array($text, $types)) {
                    throw new Exception("vCard 2.1 [$comp] [$iter]: $text is not a recognized TYPE.");
                } else {
                    $result = true;
                }
                break;
            ...
            case 'ENCODING':
                if ($text != '7BIT' &&
                    ...
                    throw new Exception("vCard 2.1 [$comp] [$iter]: $text is not a recognized ENCODING.");
                } else {
                    $result = true;
                }
                break;
            ...
            case 'VALUE':
                if ($text != 'INLINE' &&
                    ...
                    $text != 'URL' &&
                    $text != 'VCARD') {
                    throw new Exception("vCard 2.1 [$comp] [$iter]: $text is not a recognized VALUE.");
                } else {
                    $result = true;
                }
                break;
            ...
            default:
                throw new Exception("vCard 2.1 [$comp] [$iter]: $name is an unknown or invalid parameter name.");
                break;
            }
            ...
            case 'ENCODING':
                if ($text != '8BIT' &&
                    $text != 'B') {
                    throw new Exception("vCard 3.0 [$comp] [$iter]: The only allowed ENCODING parameters are 8BIT and B.");
                } else {
                    $result = true;
                }
                break;
            ...
            case 'VALUE':
                if ($text != 'BINARY' &&
                    ...
                    $text != 'UTC-OFFSET' &&
                    $text != 'VCARD') {
                    throw new Exception("vCard 3.0 [$comp] [$iter]: The only allowed VALUE parameters are BINARY, PHONE-NUMBER, TEXT, URI, UTC-OFFSET, and VCARD.");
                } else {
                    $result = true;
                }
                break;
            ...
            default:
                throw new Exception("vCard 3.0 [$comp] [$iter]: Unknown or invalid parameter name ($name).");
                break;
            ...
            throw new Exception("[$comp] [$iter] Unknown vCard version number or other error.");
    ...
    function setFormattedName($text = null)
    {
        ...
        if ($text === null) {
            ...
            if (is_array($this->value['N'])) {
                ...
                throw new Exception('FN not specified and N not set; cannot set FN.');
    ...
    */
    ...
    function setVersion($text = '3.0')
    {
        ...
        if ($text != '3.0' && $text != '2.1') {
            throw new Exception('Version must be 3.0 or 2.1 to be valid.');
        } else {
    ...
    function setSourceName($text = null)
    {
        ...
        if ($text === null) {
            if (is_array($this->value['SOURCE'])) {
                ...
                throw new Exception('NAME not specified and SOURCE not set; cannot set NAME.');
            }
        }
    ...
    */
    ...
    function getAddress($iter)
    {
        if (! is_integer($iter) || $iter < 0) {
            ...
            throw new Exception('ADR iteration number not valid.');
    ...
    */
    ...
    {
        if (! is_integer($iter) || $iter < 0) {
            throw new Exception('LABEL iteration number not valid.');
        } else {
    ...
    */
    ...
    {
        if (! is_integer($iter) || $iter < 0) {
            throw new Exception('TEL iteration number not valid.');
        } else {
    ...
    */
    ...
    {
        if (! is_integer($iter) || $iter < 0) {
            throw new Exception('EMAIL iteration number not valid.');
        } else {
    ...
    */
    ...
    function fetch()
    {
        ...
        if (! is_array($this->value['VERSION'])) {
            throw new Exception('VERSION not set (required).');
        }
        
        // FN component is required
        if (! is_array($this->value['FN'])) {
            throw new Exception('FN component not set (required).');
        }
        
        // N component is required
        if (! is_array($this->value['N'])) {
            throw new Exception('N component not set (required).');
        }
